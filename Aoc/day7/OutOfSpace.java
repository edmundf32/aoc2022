...
public  class OutOfSpace {
        ...
        Map<String, OutOfSpace> folders = new HashMap<>();
        private int size = -1;

        OutOfSpace(OutOfSpace parent, String name) {
            this.parent = parent;
            this.name = name;
        }
        ...
        public int size() {
            if (this.size == -1) {
                size = 0;
                for (OutOfSpace subfolder : folders.values()) {
                    size += subfolder.size();
                }
        ...
        public List<OutOfSpace> foldersSmallerThan(int maxSize) {
            List<OutOfSpace> smallFolders = new ArrayList<>();
            for (OutOfSpace subfolder : folders.values()) {
        ...
        public List<OutOfSpace> foldersLargerThan(int minSize) {
            List<OutOfSpace> largeFolders = new ArrayList<>();
            for (OutOfSpace subfolder : folders.values()) {
    ...
    private static OutOfSpace readInput() throws IOException {
        List<String> inputData = FileUtils.readLines(new File("C:\\Users\\Ed\\aoc2022\\Aoc\\day7\\data.txt"), Charset.defaultCharset());

        OutOfSpace rootFolder = new OutOfSpace(null, "/");
        OutOfSpace currentFolder = rootFolder;
        ...
        for (String line : inputData) {
            ...
            if (parts[0].equals("$")) {
                    ...
                    case "cd":
                        if (parts[2].equals("/")) {
                            ...
                            if (!currentFolder.folders.containsKey(parts[2])) {
                                currentFolder.folders.put(parts[2], new OutOfSpace(currentFolder, parts[2]));
                            }
                ...
                if (!currentFolder.folders.containsKey(parts[1])) {
                    currentFolder.folders.put(parts[1], new OutOfSpace(currentFolder, parts[1]));
                }
            } else {
    ...
    public static void partOne() throws IOException {
        OutOfSpace rootFolder = readInput();
        int totalSize = 0;
        List<OutOfSpace> smallFolders = rootFolder.foldersSmallerThan(100000);
        for (OutOfSpace smallFolder : smallFolders) {
    ...
    public static void partTwo() throws IOException {
        ...
        List<OutOfSpace> options = rootFolder.foldersLargerThan(toDelete);
        OutOfSpace smallest = options.get(0);
        for (OutOfSpace option : options) {
